...
const getSpecifiedMovie = async (movieID) => {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_TMDB_SERVER}/3/movie/${movieID}?api_key=${process.env.NEXT_PUBLIC_TMDB_API}&language=en-US`,
      {
        cache: 'no-cache',
      }
    )
...
export { getPopularMedia, getMovieGenres, getCategoryMovies, getSpecifiedMovie }
